...
# 보정계수
def temperature_cf(T, T_opt, T_sigma):
    f_T = np.exp(-((T-T_opt)**2) / (2 * (T_sigma)**2))
    return f_T
...
def pH_cf(pH, pH_opt, pH_sigma):
    f_pH = np.exp(-((pH - pH_opt)**2)/ (2 * (pH_sigma)**2))
    return f_pH
...
# 유효 생장률
def effective_growth(r, f_T, f_I, f_pH, f_M):
    reff = r * f_T * f_I * f_pH * f_M
    return reff


#로지스틱 생장 모델
def growth_model(X_0, K, reff, t):
    X_t = K / (1 + ((K - X_0) / X_0) * np.exp(-reff * t))
    return X_t
    
# 생장 곡선 기반 수확시점 계산식
def harvest_time(reff, X_0, K, alpha):
    t_harvest = (1 /reff) * np.log( ((K - X_0) / X_0) * (1 / (1/alpha - 1)) )
    return t_harvest
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # streamlit 파라미터
        alpha = st.slider("수확 목표 비율 (a)", 0.5, 1.0, 0.7, step=0.01)
        T = st.slider("온도(℃)", 20, 40, 32)
        I = st.slider("광도(µmol/m²/s)", 0, 300, 100)
        pH = st.slider("pH농도", 7.0, 11.0, 9.5)
        M = st.radio("교반 유무", ("O", "X"))
        X_0 = (df["X1"].iloc[0])
        K = (df["X2"].max())
        ...
        #로지스틱 계산 변수
        # early_phase_end = max(3, len(df)//3)
        # r = df["growth_biomass"].iloc[:early_phase_end].mean()
        r = df["growth_biomass"].iloc[:2].mean()
        f_T = temperature_cf(T, T_opt = 32, T_sigma = 4)
        f_I = light_cf(I)
        f_pH = pH_cf(pH, pH_opt=9.5, pH_sigma=0.5)
        f_M = mixxing(M) 
        reff = effective_growth(r, f_T, f_I, f_pH, f_M)
          
         
        
        
        #로지스틱 모델 계산
        X_t_predict = growth_model(X_0, K, reff, t_predict)
        t_harvest = harvest_time(reff, X_0, K, alpha)
        ...
        #streamlit 결과 출력
        st.write(f"보정된 유효 생장률(reff) : {reff:.4f}")
        st.write(f"예상 수확시점(일) : {t_harvest:.2f}일")
        
        
    #streamlit run c:/code/ppp2025/project/test01.py
